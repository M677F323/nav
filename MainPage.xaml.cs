...
public partial class MainPage : ContentPage
{
    ...
    void OnSelectNumber(object sender, EventArgs e)
    {
        ...
        if ((this.result.Text == "0" && pressed == "0")
    ...
    void OnCalculate(object sender, EventArgs e)
    {
        ...
        {
            if (secondNumber == 0)
                LockNumberValue(this.result.Text);

            double result = Calculate.calculate(firstNumber, secondNumber, mathOperator);

            this.CurrentCalculation.Text = $"{firstNumber} {mathOperator} {secondNumber}";

            this.result.Text = result.ToTrimmedString(decimalFormat);
            firstNumber = result;
            secondNumber = 0;
            currentState = -1;
            currentEntry = string.Empty;
